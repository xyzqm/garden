...
We can show that the condition is satisfied iff ${(n + 1)pi} < {pi}$, where ${x}$ denotes the fractional part of $x$. Using the fact that $pi ~ 22/7$ and also $pi < 22/7$, we get that when $n$ increases by $7$, the ${(n + 1)pi}$ decreases by a little more than $0$. Since $n$ is small, this means our condition will hold for $n = 7k$, or $14$ different values.
